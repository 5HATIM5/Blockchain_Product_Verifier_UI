...
const ProductList = (props: Props) => {
  ...
  const [imageListId, setImageListId] = useState<number[]>([]);
    ...
    listAll(imageListRef).then((response) => {
      response.items.forEach((item) => {
        getDownloadURL(item).then((url) => {
          const match = url.match(/\/images%2F(\d+)\?/);
          const number = match ? parseInt(match[1]) : null;
          setImageList((prev: string[]) => [...prev, url]);
          if (number !== null) {
            setImageListId((prev: number[]) => [...prev, number]);
          }
        });
      });
    });
    ...
    <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      <div className=" grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:gap-x-8">
        {products?.map((product) => {
          const index = imageListId.findIndex((value) => value === product.id);
          return (
