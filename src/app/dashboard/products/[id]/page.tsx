...
import Loader from "@/app/ui/Components/Global/Loader";
import { GetProductsById, UpdateProduct } from "@/lib/data";
...
const SingleProductPage = (props: Props) => {
  ...
  const UpdateProductData = async (productData: FormData) => {
    ...
    setProduct(await UpdateProduct(id as string, ProductFormData));
    router.back();
  };
        ...
        >
          ...
          <div className="relative z-0 w-full mb-6 group">
            <input
              name="location"
              id="location"
              defaultValue={product.production_location}
              className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
              placeholder=" "
              required
            />
